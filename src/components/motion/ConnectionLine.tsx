...
/**
 * Build a smooth cubic-bezier path through spine anchors.
  ...
  return (
    ...
    >
      ...
      {spine.map((a, i) => {
        ...
        return <SpineDot key={a.id} cx={x} cy={y} progress={scrollYProgress} index={i} total={spine.length} reduced={!!reduced} />;
      })}

      {/* Orphan (chaos) nodes — pulse softly, never connect */}
      {!reduced && orphans.map((a) => (
        <motion.circle
          key={a.id}
          cx={a.x}
          cy={a.y}
      ...
      {!reduced && isDesktop && (
        <motion.circle
          ...
          r={3.5}
          fill="var(--primary)"
          style={{ opacity: particleOpacity, filter: "drop-shadow(0 0 6px rgba(58,93,226,0.6))" }}
        />
      )}
    </svg>
  );
...
function SpineDot({
  ...
  const scale = useTransform(progress, [threshold - 0.05, threshold, threshold + 0.05], [1, 1.6, 1]);
  return (
    <motion.circle
      cx={cx}
      cy={cy}
      ...
      style={
        reduced
          ? { opacity: 0.7 }
          : { opacity, scale, transformOrigin: `${cx}px ${cy}px` }
      }
    />
  );
}
