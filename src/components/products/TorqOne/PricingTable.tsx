...
import {
  Check, X, Star, Users, Cpu, MessageSquare, Bell,
  Shield, Crown, Zap,
} from 'lucide-react';
...
const MODULES: {
  ...
  {
    icon: Users, label: 'Customer Acquisition & CRM',
    values: [
      ['—', 'gray'],
      ['Complete', 'green'],
  ...
  {
    icon: Users, label: 'Member Journey & Success',
    values: [
      ['Basic', 'gray'],
      ['Advanced', 'blue'],
  ...
  {
    icon: Users, label: 'Trainer Workspace',
    values: [
      ['—', 'gray'],
      ['Complete', 'green'],
      ['Advanced', 'blue'],
  ...
  {
    icon: Users, label: 'Gym Operations',
    values: [
      ['Intermediate', 'blue'],
      ['Advanced', 'blue'],
  ...
  {
    icon: Users, label: 'Business Management & Billing',
    values: [
      ['Intermediate', 'blue'],
      ['Advanced', 'blue'],
      ['Advanced + Business Suite', 'green'],
  ...
  {
    icon: Users, label: 'Marketing & Growth',
    values: [
      ['Basic', 'gray'],
      ['AI Powered', 'purple'],
  ...
  {
    icon: Users, label: 'AI & Automation',
    values: [
      ['Basic AI', 'purple'],
      ['Growth AI', 'purple'],
      ['Pro AI', 'purple'],
  ...
  {
    icon: Users, label: 'Business Intelligence',
    values: [
      ['Basic', 'gray'],
      ['Intermediate', 'blue'],
  ...
  {
    icon: Users, label: 'Enterprise & Multi-Branch',
    values: [
      ['—', 'gray'],
      ['—', 'gray'],
      ['Multi-Branch', 'green'],
  ...
  {
    icon: Users, label: 'Secure Cloud Platform',
    values: [
      ['Standard', 'gray'],
      ['Enhanced', 'blue'],
      ['Premium', 'green'],
  ...
  {
    icon: Users, label: 'Data Migration',
    values: [
      ['Add-on', 'gray'],
      ['Included', 'green'],
      ['Included', 'green'],
...
const USAGE_ROWS: {
  ...
  {
    icon: Cpu, label: 'Owner AI Usage',
    values: ['100 AI Operations / month', '250 AI Operations / month', '600 AI Operations / month', 'Custom'],
  },
  {
    icon: MessageSquare, label: 'Member AI Usage',
    values: [
...
function PlanHeader({ p, compact }: { p: typeof PRICING[number]; compact?: boolean }) {
  return (
    <div className={`relative flex flex-col items-center text-center ${p.highlight ? 'py-5' : 'py-4'}`}>
      {p.highlight && (
      ...
      <div className={`flex items-center gap-1.5 ${compact ? 'mb-2' : 'mb-3'}`}>
        ...
        <span className={`font-bold text-white ${compact ? 'text-sm' : 'text-base'}`}>{p.plan}</span>
      </div>
      <div className="flex items-baseline gap-1">
        <span className={`font-extrabold text-white tracking-tight ${compact ? 'text-2xl' : 'text-3xl'}`}>{p.monthly}</span>
        {p.monthly !== 'Custom' && <span className="text-xs text-torqone-text-muted font-medium">/mo</span>}
      </div>
      <div className="mt-1.5 flex flex-col items-center gap-0.5">
        <span className="text-[11px] text-torqone-text-muted">
          Onboarding <span className="text-torqone-text-secondary font-semibold">{p.onboarding}</span>
        </span>
        <span className="text-[11px] text-torqone-text-muted">
          Members <span className="text-torqone-text-secondary font-semibold">{p.members}</span>
        </span>
      </div>
    </div>
  );
    ...
    <div className="hidden lg:block glass-card rounded-2xl overflow-hidden">
      ...
      <div className="grid grid-cols-[260px_repeat(4,1fr)] border-b border-torqone-border bg-torqone-card/40">
        <div className="p-5 flex items-center">
          <span className="text-xs font-semibold tracking-wider uppercase text-torqone-text-muted">Choose your plan</span>
        </div>
        {PRICING.map((p) => (
          <div
            key={p.plan}
            className={p.highlight ? 'bg-torqone-primary-muted border-x border-torqone-primary/30' : 'border-x border-torqone-border/50'}
          >
            <PlanHeader p={p} />
          </div>
        ))}
      ...
      <div className="grid grid-cols-[260px_repeat(4,1fr)] border-b border-torqone-border/50 bg-torqone-background/30">
        <div className="p-3.5 col-span-5 text-center">
          <p className="text-[11px] text-torqone-text-muted leading-relaxed">
            Onboarding includes platform configuration, data migration, staff setup and required integrations.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[260px_repeat(4,1fr)] border-b border-torqone-border">
        <div className="p-3.5 col-span-5">
          <span className="text-[11px] font-bold tracking-wider uppercase text-torqone-accent">Module Availability</span>
        </div>
      </div>
        ...
        >
          <div className="p-4 sticky left-0 z-10 bg-inherit flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-torqone-card flex items-center justify-center shrink-0 border border-torqone-border">
              <mod.icon className="w-4 h-4 text-torqone-text-secondary" strokeWidth={1.75} />
            </div>
            <span className="text-sm font-medium text-torqone-text-secondary">{mod.label}</span>
          </div>
          {mod.values.map((v, j) => (
            <div
              key={j}
      ...
      <div className="grid grid-cols-[260px_repeat(4,1fr)] border-b border-torqone-border bg-torqone-card/30">
        <div className="p-3.5 col-span-5">
          <span className="text-[11px] font-bold tracking-wider uppercase text-torqone-accent">Usage & Limits</span>
        </div>
      </div>
      ...
      {USAGE_ROWS.map((row, i) => (
        ...
        >
          <div className="p-4 sticky left-0 z-10 bg-inherit flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-torqone-card flex items-center justify-center shrink-0 border border-torqone-border">
              <row.icon className="w-4 h-4 text-torqone-text-secondary" strokeWidth={1.75} />
            </div>
          ...
          {row.values.map((v, j) => (
            ...
            >
              <span className="text-xs font-medium text-torqone-text-secondary leading-relaxed">{v}</span>
            </div>
          ))}
        </motion.div>
      ))}
  ...
  return (
    <div className="lg:hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* Plan header */}
          <div className={`p-5 text-center border-b border-torqone-border ${PRICING[active].highlight ? 'bg-torqone-primary-muted' : 'bg-torqone-card/40'}`}>
          ...
          {/* Modules */}
          <div className="p-4">
            <span className="text-[11px] font-bold tracking-wider uppercase text-torqone-accent block mb-3">Module Availability</span>
                ...
                <div key={mod.label} className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-7 h-7 rounded-lg bg-torqone-card flex items-center justify-center shrink-0 border border-torqone-border">
                      <mod.icon className="w-3.5 h-3.5 text-torqone-text-secondary" strokeWidth={1.75} />
                    </div>
                    <span className="text-xs font-medium text-torqone-text-secondary truncate">{mod.label}</span>
                  </div>
                  <StatusPill label={mod.values[active][0]} kind={mod.values[active][1]} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 border-t border-torqone-border/50">
            <span className="text-[11px] font-bold tracking-wider uppercase text-torqone-accent block mb-3">Usage & Limits</span>
            <div className="space-y-2.5">
              {USAGE_ROWS.map((row) => (
                <div key={row.label} className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2.5 shrink-0">
                    <div className="w-7 h-7 rounded-lg bg-torqone-card flex items-center justify-center shrink-0 border border-torqone-border">
                      <row.icon className="w-3.5 h-3.5 text-torqone-text-secondary" strokeWidth={1.75} />
                    </div>
                    <span className="text-xs font-medium text-torqone-text-secondary">{row.label}</span>
                  </div>
                  <span className="text-xs font-semibold text-torqone-text text-right">{row.values[active]}</span>
                </div>
              ))}
            </div>
          </div>
...
function Legend() {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-4 mt-6">
      {items.map((it) => (
        <span key={it.label} className="flex items-center gap-1.5 text-xs text-torqone-text-muted">
          <span className="w-3 h-3 rounded-full border" style={{ background: PILL_STYLES[it.kind].bg, borderColor: PILL_STYLES[it.kind].border }} />
          {it.label}
        </span>
      ))}
    </div>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <SectionHeading
            ...
            title={<>One platform. <GradientText>Four ways to grow.</GradientText></>}
