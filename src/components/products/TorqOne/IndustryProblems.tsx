...
import {
  TrendingDown, Users, Wrench, Gift, HeartHandshake, AlertTriangle, Layers, EyeOff, MessageSquareOff, DollarSign, BrickWall, Building2, ShieldAlert, CalendarOff, BarChart3, PhoneCall, MessageSquareMore
} from 'lucide-react';
...
const HIDDEN_PROBLEMS = [
  ...
  },
  {
    ...
    border: 'border-torqone-success/20',
    title: 'Happy Members Rarely Become New Members',
    body: ["People finish great workouts, hit personal milestones, and recommend your gym in conversations—but most of those moments are never turned into reviews, referrals, guest visits, or new memberships. ",
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <SectionHeading
              ...
              title={<>You know something's wrong.<br /><GradientText>You just can't fix it alone.</GradientText></>}
          ...
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {OBVIOUS_PROBLEMS.map((p) => (
              <StaggerItem key={p.title}>
                ...
                >
                  <div className={`inline-flex h-10 w-10 items-center justify-center rounded-xl ${p.bg} border ${p.border} mb-4`}>
                    <p.icon className={`w-5 h-5 ${p.color}`} />
                  </div>
                  <h3 className="text-base font-semibold text-white mb-2">{p.title}</h3>
                  <ul className="space-y-2">
                    {p.body.map((statement) => (
                      <li key={statement} className="text-sm text-torqone-text-secondary leading-relaxed">{statement}</li>
                    ))}
                  </ul>
                </motion.div>
              </StaggerItem>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <SectionHeading
              ...
              title={<>But there's more.<br /><GradientText>The problems you've accepted as normal.</GradientText></>}
                  ...
                  <ul className="space-y-2">
                    {
                      p.body.map((statement, index)=> (
                            <li key={index} className="text-sm text-torqone-text-secondary leading-relaxed">{statement}</li>
                      ))
                    }
                  </ul>
                </motion.div>
