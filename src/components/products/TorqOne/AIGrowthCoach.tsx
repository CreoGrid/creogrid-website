...
import {
  Brain, Sparkles, Megaphone, Target, Apple, BarChart3, Bell,
  ArrowRight, BrainCircuit, Radar, Dumbbell, LineChart
} from 'lucide-react';
...
const AI_CARDS = [
  {
    ...
    description: 'Your executive AI advisor that continuously analyzes your business, prioritizes opportunities and delivers practical actions every week.',
    features: [
  ...
  },
  {
    ...
    description: 'Continuously monitors every critical area of your business in real time so problems are identified before they become expensive.',
    features: [
      'Member Activity',
      'Attendance',
      'Leads',
  ...
  },
  {
    ...
    description: 'Predicts future business outcomes using patterns across your gym so you can act before problems happen.',
    features: [
      'Members Likely To Leave',
      'Members Likely To Renew',
      'Revenue Trends',
  ...
  },
  {
    ...
    description: 'Turns enquiries into paying members using intelligent scoring, follow-ups and conversion optimization.',
    features: [
      'AI Lead Scoring',
  ...
  },
  {
    ...
    description: 'Creates, optimizes and continuously improves marketing campaigns across multiple channels.',
    features: [
      'Facebook Campaigns',
      'Instagram Campaigns',
      'Posters',
  ...
  },
  {
    ...
    description: 'Keeps members engaged, motivated and progressing toward real fitness results.',
    features: [
      'AI Workout Guidance',
      'AI Nutrition Plans',
  ...
  },
  {
    ...
    description: 'Transforms business data into executive-level insights for smarter decisions and long-term growth.',
    features: [
...
function AIProcessingPanel() {
  const steps = [
    ...
    { label: 'Scoring churn risk (risk of member leaving) and predicting recovery probability', status: 'running' },
      ...
      <div>
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-torqone-border/60">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-white">AI Business Growth Coach</span>
          </div>
          <Badge variant="ai" dot pulse size="sm">Processing</Badge>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            {steps.map((step, i) => (
              ...
              >
                <div className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center border ${step.status === 'done'
                  ...
                  {step.status === 'done' && (
                    <svg className="w-3 h-3 text-torqone-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                ...
                <span className={`text-xs ${step.status === 'done' ? 'text-torqone-text-secondary line-through opacity-60' :
                  step.status === 'running' ? 'text-white font-medium' : 'text-torqone-text-muted'
                }`}>
                  {step.label}
                </span>
              </motion.div>
          ...
          <div className="rounded-xl border border-torqone-ai/30 bg-torqone-ai-muted p-4">
            <p className="text-xs font-semibold text-torqone-ai mb-2">Latest Insight Generated</p>
            <p className="text-xs text-torqone-text-secondary leading-relaxed">
              <span className="text-white font-semibold">17 members</span> haven&apos;t visited in 9 days. WhatsApp re-engagement sent automatically. <span className="text-white font-semibold">4 already rebooked</span>. Projected recovery: <span className="text-torqone-success font-semibold">₹4,140/month</span>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 pt-0">
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={m.label} className="rounded-lg bg-torqone-background/50 border border-torqone-border/40 p-2.5 text-center">
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="ai" dot pulse size="sm">⭐ Hero Module</Badge>
          </div>
          <span className="text-xs text-torqone-text-muted font-mono">torqone.ai / growth-coach</span>
        </div>
      ...
      <div className="mt-auto border-t border-torqone-border/40 pt-6">
        <p className="text-xs font-semibold text-torqone-ai uppercase tracking-wider mb-4">Core Capabilities</p>
          ...
          <div className="space-y-3">
            {horizontalChunks.map((chunk, chunkIdx) => (
              <div key={chunkIdx} className="flex flex-wrap items-center gap-x-5">
                {chunk.map((feat) => (
                  <span key={feat} className="inline-flex items-center text-xs text-torqone-text-secondary font-medium">
                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 mr-2 ${colors.bullet}`} />
                    {feat}
                  </span>
                ))}
              </div>
        ...
        {vertical.length > 0 && (
          <ul className={`${horizontalChunks.length > 0 ? 'mt-4' : ''} space-y-2.5`}>
            {vertical.map((feat) => (
              <li key={feat} className="flex items-start gap-2.5 text-xs text-torqone-text-secondary leading-relaxed">
                <span className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${colors.bullet}`} />
                <span>{feat}</span>
              </li>
            ))}
          </ul>
...
function AICapabilityCard({ card }: { card: typeof AI_CARDS[0] }) {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute -top-12 -right-12 w-24 h-24 rounded-full blur-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-300 bg-gradient-to-br ${colors.gradient}`} />

      <div>
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <div className={`w-9 h-9 rounded-xl ${colors.bgMuted} border ${colors.border} flex items-center justify-center`}>
            <card.icon className={`w-5 h-5 ${colors.text}`} />
          </div>
          <span className="text-[10px] text-torqone-text-muted font-mono">torqone.ai / {card.id}</span>
        </div>
        ...
        {/* Description */}
        <p className="text-xs text-torqone-text-secondary leading-relaxed mb-4">{card.description}</p>
      </div>
      ...
      <div className="mt-auto border-t border-torqone-border/40 pt-4">
          ...
          <div className="space-y-2">
            {horizontalChunks.map((chunk, chunkIdx) => (
              <div key={chunkIdx} className="flex flex-wrap items-center gap-x-4">
                {chunk.map((feat) => (
                  <span key={feat} className="inline-flex items-center text-xs text-torqone-text-secondary">
                    <span className={`w-1 h-1 rounded-full shrink-0 mr-1.5 ${colors.bullet}`} />
                    {feat}
                  </span>
                ))}
              </div>
        ...
        {vertical.length > 0 && (
          <ul className={`${horizontalChunks.length > 0 ? 'mt-3' : ''} space-y-2`}>
            {vertical.map((feat) => (
              <li key={feat} className="flex items-start gap-2 text-xs text-torqone-text-secondary leading-relaxed">
                <span className={`mt-1.5 w-1 h-1 rounded-full shrink-0 ${colors.bullet}`} />
                <span>{feat}</span>
              </li>
            ))}
          </ul>
    ...
    <section className="relative py-28 overflow-hidden bg-torqone-card/10" id="ai-engine">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse at top, #7B5CFF 0%, #00D4FF 50%, transparent 80%)' }}
        />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          ...
          <SectionHeading
            eyebrow="AI Engine"
            title={<>What if you had a AI business coach<br /><GradientText variant="ai">who never sleeps?</GradientText></>}
