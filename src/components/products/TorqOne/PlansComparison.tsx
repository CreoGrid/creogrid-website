...
function StatusPill({ value }: { value: string }) {
  ...
  return (
    ...
    >
      {value === '—' ? (
        <span className="text-torqone-text-muted/50">—</span>
      ) : (
        value
      )}
    </span>
...
const PLANS: Plan[] = [
  { id: 'launch', name: 'Launch', monthly: '₹1499', onboarding: '₹25k', highlighted: false, accent: '#3A5DE2' },
  { id: 'growth', name: 'Growth (recommended)', monthly: '₹2,699', onboarding: '₹35k', highlighted: true, accent: '#7B5CFF' },
  { id: 'scale', name: 'Scale', monthly: '₹4,999', onboarding: '₹50k', highlighted: false, accent: '#00D4FF' },
  { id: 'enterprise', name: 'Enterprise', monthly: 'Custom', onboarding: 'Custom', highlighted: false, accent: '#F59E0B' },
];
...
const MODULES: ModuleRow[] = [
  { icon: Users, name: 'Member Management', values: ['Intermediate', 'Advanced', 'Complete', 'Enterprise-level'] },
  { icon: Magnet, name: 'Customer Acquisition & CRM', values: ['—', 'Complete', 'Complete + AI Enhanced', 'Enterprise-level'] },
  { icon: HeartPulse, name: 'Member Journey & Success', values: ['Basic', 'Advanced', 'Advanced + Pro AI', 'Enterprise-level'] },
  { icon: GraduationCap, name: 'Trainer Workspace', values: ['—', 'Complete', 'Advanced', 'Enterprise-level'] },
  { icon: Dumbbell, name: 'Gym Operations', values: ['Intermediate', 'Advanced', 'Advanced + Business Suite', 'Enterprise-level'] },
  { icon: Wallet, name: 'Business Management & Billing', values: ['Intermediate', 'Advanced', 'Advanced + Business Suite', 'Enterprise-level'] },
  { icon: Megaphone, name: 'Marketing & Growth', values: ['Basic', 'AI Powered', 'AI Powered + Self-Learning AI', 'Enterprise-level'] },
  { icon: Bot, name: 'AI & Automation', values: ['Basic AI', 'Growth AI', 'Pro AI', 'Custom AI'] },
  { icon: BarChart3, name: 'Business Intelligence', values: ['Basic', 'Intermediate', 'Executive + AI Powered', 'Enterprise-level'] },
  { icon: Network, name: 'Enterprise & Multi-Branch', values: ['—', '—', 'Multi-Branch', 'Unlimited'] },
  { icon: Cloud, name: 'Secure Cloud Platform', values: ['Standard', 'Enhanced', 'Premium', 'Dedicated'] },
  { icon: RefreshCw, name: 'Data Migration', values: ['Add-on', 'Included', 'Included', 'Priority'] },
];
...
const USAGE: UsageRow[] = [
  { icon: Users, label: 'Members', aiAccent: false, values: ['Up to 100', '101–300', '301–500', '500+'] },
  { icon: Sparkles, label: 'Owner AI Usage', aiAccent: true, values: ['100 AI Operations / month', '250 AI Operations / month', '600 AI Operations / month', 'Custom'] },
  { icon: MessageSquare, label: 'Member AI Usage', aiAccent: true, values: ['20 AI Conversations / active member / month', '30 AI Conversations / active member / month', '40 AI Conversations / active member / month', 'Custom'] },
  { icon: MessageCircle, label: 'WhatsApp Notifications', aiAccent: false, values: ['1,000 / month', '5,000 / month', '15,000 / month', 'Custom / Fair Usage'] },
];
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <SectionHeading
            ...
            title={<>Compare every plan, <GradientText>module by module.</GradientText></>}
            ...
            <div className="glass-card rounded-2xl overflow-hidden">
              <div className={`grid ${GRID} min-w-[920px]`}>
                ...
                <div className="sticky left-0 z-20 bg-torqone-card border-r border-torqone-border p-5">
                  <span className="text-xs font-semibold tracking-wider uppercase text-torqone-text-muted">Core Module</span>
                </div>
                {PLANS.map((plan) => (
                  <div
                    key={plan.id}
                  ...
                  >
                    ...
                    <div className="flex items-baseline justify-center gap-1">
                      <span className="text-2xl font-extrabold text-white tracking-tight">{plan.monthly}</span>
                      <span className="text-xs text-torqone-text-muted">/mo</span>
                    </div>
                    {/* Onboarding — secondary */}
                    <p className="text-[11px] text-torqone-text-muted mt-1">
                      Onboarding <span className="text-torqone-text-secondary font-semibold">{plan.onboarding}</span>
                    </p>
                  </div>
                ))}
                ...
                <div className="col-span-5 px-5 py-3.5 bg-torqone-background/40 border-y border-torqone-border/60">
                  <p className="text-xs text-torqone-text-muted leading-relaxed text-center">
                    Onboarding includes platform configuration, staff setup and required integrations.{' '}
                    <span className="text-torqone-text-secondary">
                      Data migration is included from Growth onward and available as an add-on for Launch.
                    </span>
                  </p>
                </div>
                ...
                {MODULES.map((mod, i) => (
                  <div key={mod.name} className="contents">
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-torqone-text-secondary">{mod.name}</span>
                    </div>
                    {mod.values.map((v, j) => (
                      <div
                        key={j}
                        className={`p-4 flex items-center justify-center text-center border-x border-torqone-border/40 border-t border-torqone-border/60 ${
                          PLANS[j].highlighted ? 'bg-torqone-ai-muted/20' : i % 2 === 0 ? 'bg-torqone-background/25' : ''
                        }`}
                      >
                        <StatusPill value={v} />
                      </div>
                ...
                <div className="col-span-5 px-5 py-3 bg-torqone-background/50 border-t border-torqone-border">
                  <span className="text-xs font-semibold tracking-wider uppercase text-torqone-accent">Usage & Limits</span>
                </div>
                ...
                {USAGE.map((row, i) => (
                  <div key={row.label} className="contents">
                    ...
                    >
                      <row.icon className={`w-4 h-4 shrink-0 ${row.aiAccent ? 'text-torqone-ai' : 'text-torqone-accent'}`} />
                      <span className="text-sm font-medium text-torqone-text-secondary">{row.label}</span>
                    </div>
                    {row.values.map((v, j) => (
                      <div
                        key={j}
                      ...
                      >
                        <span className={`text-xs font-medium leading-snug ${row.aiAccent ? 'text-torqone-ai' : 'text-torqone-text-secondary'}`}>
                          {v}
                        </span>
                      </div>
                    ))}
          ...
          <div className="lg:hidden">
            ...
            <div className="flex items-center justify-center gap-2 mb-6">
              {PLANS.map((p, i) => (
                ...
                >
                  {p.highlighted && <Star className="w-3 h-3 inline mr-1 fill-torqone-ai text-torqone-ai" />}
                  {p.name}
                </button>
              ))}
            </div>
              ...
              {PLANS.map((plan) => (
                <div
                  ...
                  className={`snap-center shrink-0 w-[88%] glass-card rounded-2xl p-5 ${
                    plan.highlighted ? 'shadow-torqone-ai border-torqone-ai/30' : ''
                  }`}
                >
                  {/* Plan header */}
                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-torqone-border/60">
                    <div className="flex items-center gap-2">
                      {plan.highlighted && <Star className="w-4 h-4 fill-torqone-ai text-torqone-ai" />}
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-torqone-text-muted">Onboarding {plan.onboarding}</p>
                    </div>
                  </div>

                  {/* Onboarding note */}
                  <p className="text-[11px] text-torqone-text-muted leading-relaxed mb-4 p-2.5 rounded-lg bg-torqone-background/40">
                    Onboarding includes platform configuration, staff setup and required integrations. Data migration is included from Growth onward.
                  </p>
                  ...
                  <div className="space-y-2.5 mb-5">
                    {MODULES.map((mod) => {
                      ...
                      return (
                        <div key={mod.name} className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2 min-w-0">
                            <mod.icon className="w-4 h-4 text-torqone-accent shrink-0" />
                            <span className="text-xs font-medium text-torqone-text-secondary truncate">{mod.name}</span>
                          </div>
                  ...
                  <div className="pt-4 border-t border-torqone-border/60">
                    <p className="text-xs font-semibold tracking-wider uppercase text-torqone-accent mb-3">Usage & Limits</p>
                      ...
                      {USAGE.map((row) => {
                        ...
                        return (
                          <div key={row.label} className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2 min-w-0">
                              <row.icon className={`w-4 h-4 shrink-0 ${row.aiAccent ? 'text-torqone-ai' : 'text-torqone-accent'}`} />
                              <span className="text-xs font-medium text-torqone-text-secondary">{row.label}</span>
                            </div>
                            <span className={`text-xs font-semibold text-right max-w-[55%] ${row.aiAccent ? 'text-torqone-ai' : 'text-torqone-text-secondary'}`}>
                              {row.values[idx]}
                            </span>
                          </div>
                        );
              ...
              {PLANS.map((p, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    activeIdx === i ? 'w-6 bg-torqone-ai' : 'w-1.5 bg-torqone-border'
                  }`}
                />
              ))}
            </div>
