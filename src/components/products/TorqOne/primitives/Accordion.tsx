      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            >
              <span className={cn('text-sm font-semibold leading-snug', isOpen ? 'text-white' : 'text-torqone-text-secondary')}>
                {item.question}
              </span>
              <motion.span
                animate={{ rotate: isOpen ? 180 : 0 }}
                transition={{ duration: 0.25, ease: 'easeInOut' }}
                className={cn('shrink-0 text-torqone-text-muted transition-colors', isOpen && 'text-torqone-primary')}
              >
